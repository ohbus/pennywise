...
class GroupControllerTest {
    ...
    fun `creates and lists a group for subject`() {
        mvc.perform(post("/expense-core/v1/groups").with(alice).contentType(MediaType.APPLICATION_JSON)
            .content("{\"name\":\"Vienna trip\",\"kind\":\"TRIP\",\"currency\":\"EUR\"}"))
            .andExpect(status().isCreated).andExpect(jsonPath("$.name").value("Vienna trip"))
        mvc.perform(get("/expense-core/v1/groups").with(alice))
    ...
    fun `rejects invalid group request`() {
        val result = mvc.perform(post("/expense-core/v1/groups").with(alice).contentType(MediaType.APPLICATION_JSON)
    ...
    fun `creates and claims invitation only once`() {
        val created = mvc.perform(post("/expense-core/v1/groups").with(alice).contentType(MediaType.APPLICATION_JSON)
            .content("{\"name\":\"Trip\",\"kind\":\"TRIP\",\"currency\":\"EUR\"}"))
            .andReturn().response.contentAsString
        val groupId = Regex("\\\"groupId\\\":\\\"([^\\\"]+)\\\"").find(created)!!.groupValues[1]
        val invite = mvc.perform(post("/expense-core/v1/groups/$groupId/invites").with(alice).contentType(MediaType.APPLICATION_JSON)
    ...
    fun `gets group by id for member and returns 404 for non-existent or non-member`() {
        ...
        mvc.perform(get("/expense-core/v1/groups/$groupId").with(alice))
        ...
        mvc.perform(get("/expense-core/v1/groups/$groupId").with(bob))
    ...
    fun `lists members for group member and returns 404 for non-member`() {
        ...
        mvc.perform(get("/expense-core/v1/groups/$groupId/members").with(alice))
            .andExpect(status().isOk)
        ...
        val bob = RequestPostProcessor { request -> request.userPrincipal = Principal { "bob" }; request }
        mvc.perform(get("/expense-core/v1/groups/$groupId/members").with(bob))
            .andExpect(status().isNotFound)
    }
    ...
    fun `lists all group members after invite claimed`() {
        ...
        val created = testMvc.perform(post("/expense-core/v1/groups").with(alice).contentType(MediaType.APPLICATION_JSON)
            .content("{\"name\":\"Paris Trip\",\"kind\":\"TRIP\",\"currency\":\"EUR\"}"))
            .andExpect(status().isCreated).andReturn().response.contentAsString
        val groupId = Regex("\\\"groupId\\\":\\\"([^\\\"]+)\\\"").find(created)!!.groupValues[1]

        val invite = testMvc.perform(post("/expense-core/v1/groups/$groupId/invites").with(alice).contentType(MediaType.APPLICATION_JSON)
            .content("{\"expiresInHours\":24}"))
            .andExpect(status().isCreated).andReturn().response.contentAsString
        val token = Regex("\\\"token\\\":\\\"([^\\\"]+)\\\"").find(invite)!!.groupValues[1]

        val bob = RequestPostProcessor { request -> request.userPrincipal = Principal { "bob" }; request }
        testMvc.perform(post("/expense-core/v1/invites/$token/claim").with(bob))
            .andExpect(status().isOk)

        testMvc.perform(get("/expense-core/v1/groups/$groupId/members").with(bob))
            .andExpect(status().isOk)
    ...
    fun `renames group via PATCH and increments revision`() {
        ...
        mvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.patch("/expense-core/v1/groups/$groupId").with(alice)
    ...
    fun `renames group with unicode name`() {
        ...
        val requestJson = org.springframework.test.web.servlet.request.MockMvcRequestBuilders.patch("/expense-core/v1/groups/$groupId")
            .with(alice)
    ...
    fun `renames group at boundary length 120 succeeds and 121 fails validation`() {
        ...
        val rejectedResult = mvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.patch("/expense-core/v1/groups/$groupId").with(alice)
    ...
    fun `returns 404 when group does not exist`() {
        ...
        mvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.patch("/expense-core/v1/groups/$nonExistentGroupId").with(alice)
            .contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"New Name\"}"))
            .andExpect(status().isNotFound)
    ...
    fun `returns 404 when non-member attempts group rename`() {
        ...
        val bob = RequestPostProcessor { request -> request.userPrincipal = Principal { "bob" }; request }
        mvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.patch("/expense-core/v1/groups/$groupId").with(bob)
