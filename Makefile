...
bruno-run: ## Run the ordered Bruno collection; override BRUNO_ENV, BRUNO_TOKEN, and BRUNO_REPORT
	...
	@report="$${BRUNO_REPORT:-/tmp/pennywise-bruno-$$(date +%s).json}"; mkdir -p "$$(dirname "$$report")"; \
		(cd tools/bruno && npx --yes @usebruno/cli run accounts bff expense-core/groups expense-core/expenses expense-core/settlements expense-core/sync expense-core/recurring notifications expense-core/lifecycle quality -r --env "$${BRUNO_ENV:-local}" --env-var "token=$${BRUNO_TOKEN:-test-user}" --reporter-json "$$report" --reporter-skip-headers --reporter-skip-body); \
