...
class DbTelemetryTest {
    ...
    fun `retains fallback and failure diagnostics without a metrics registry`() {
        ...
        assertEquals(DbTelemetrySnapshot(fallbacks = 1, failures = 1, acquisitions = 1, acquisitionTotalMs = 3, lockWaits = 1, deadlocks = 1, queries = 1, queryTotalMs = 120, slowQueries = 1), telemetry.snapshot())
    }
}
