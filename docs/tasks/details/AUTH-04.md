...
- Real Keycloak issuer availability and signed-token REST/GraphQL HTTP journeys
  ...
  wrong-audience rejection is now verified across all four HTTP boundaries.
  Wrong-issuer, expiry, signature, algorithm, subject, and WebSocket invalid-
  token cases remain open and must not be inferred from the happy path.
